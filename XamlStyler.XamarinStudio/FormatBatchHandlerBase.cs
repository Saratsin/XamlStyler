...
{
	public abstract class FormatBatchHandlerBase : FormatXamlHandler
	{
		...
		private void BatchProcessProject(Project prj, StylerOptions options, StylerService styler)
		{
			...
			foreach (var file in prj.Files.Where(IsProjectFileSupported).ToArray())
			{
		...
		protected virtual bool IsProjectFileSupported(ProjectFile file)
		{
			return file.Name.EndsWithAny(Config.FileExtensions, System.StringComparison.OrdinalIgnoreCase);
		}
